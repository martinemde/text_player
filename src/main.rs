...
use text_player::{Formatters, Gamefile, Session};

#[derive(Parser)]
#[command(name = "text_player")]
    ...
    let (game_name, formatter_name, dfrotz_path) = match cli.command {
        ...
        None => {
            if let Some(game) = cli.game {
                ...
                eprintln!("Usage: text_player [GAME] or text_player play [GAME]");
                std::process::exit(1);
            }
        }
    };
...
fn run_game(game_name: &str, formatter_name: &str, dfrotz_path: Option<String>) -> text_player::Result<()> {
    // Find the game file
    let gamefile = Gamefile::from_input(game_name)?;

    if !gamefile.exists() {
        return Err(text_player::Error::GameNotFound(game_name.to_string()));
    }
