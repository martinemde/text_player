...
impl ShellFormatter {
    fn format_system_feedback(&self, result: &CommandResult) -> String {
        ...
        let prefix = if result.success { "\x1b[32m✓\x1b[0m" } else { "\x1b[31m✗\x1b[0m" };
        let message = result.message.as_ref().map(|s| s.as_str()).unwrap_or("");
        let mut feedback = format!("{} {}: {}", prefix, result.operation.to_string().to_uppercase(), message);

        // Add details if present
        if !result.details.is_empty() {
            let detail_lines: Vec<String> = result.details.iter()
                .map(|(k, v)| format!("  {}: {}", k, v))
                .collect();
    ...
    fn format_game_output(&self, result: &CommandResult) -> String {
        result.message.as_ref().unwrap_or(&result.raw_output).clone()
    }
...
impl DataFormatter {
    fn extract_location(&self, text: &str) -> Option<String> {
        ...
        let location = self.extract_location_with_stats(first_line)
