...
impl ActionCommand {
    fn failure_detected(output: &str) -> bool {
        FAILURE_PATTERNS.iter().any(|pattern| pattern.is_match(output))
    }
}
...
impl Command for SaveCommand {
    fn execute(&self, game: &mut Dfrotz) -> Result<CommandResult> {
        ...
        let mut result = game.read_until(Some(&Regex::new(r"(?i)Overwrite existing file\? |Ok\.|Failed\.|>").unwrap()))?;
...
impl Command for RestoreCommand {
    fn execute(&self, game: &mut Dfrotz) -> Result<CommandResult> {
        ...
        let success = result.contains("Ok.");
        let message = if success {
            Some(format!("[{}] Game restored successfully", self.savefile.slot))
        } else {
